(() => {
  ...
  function buildMessage(form, packageKey, selectedOrderType, page) {
    ...
    const lines = [
      ...
      `Tema/Briefing: ${String(data.get("briefing") || "").trim()}`,
      `Cores principais: ${String(data.get("coresPrincipais") || "").trim()}`,
      `Logo (URL): ${String(data.get("logoUrl") || "").trim()}`,
      `Frase/nome no biscoito: ${withFallback(String(data.get("fraseNoBiscoito") || "").trim())}`,
